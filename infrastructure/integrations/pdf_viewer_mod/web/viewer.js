...
const PDFViewerApplication = {
  ...
  _forceCssTheme() {
    ...
    // MODIFICADO: Permitir temas personalizados inyectados desde Python
    if (window.CUSTOM_PDF_THEME !== undefined) {
      // Si tenemos un tema personalizado, úsalo
      const customTheme = window.CUSTOM_PDF_THEME;
      if (customTheme === 'light' || customTheme === 1) {
        this._applyCssTheme(ViewerCssTheme.LIGHT);
        return;
      } else if (customTheme === 'dark' || customTheme === 2) {
        this._applyCssTheme(ViewerCssTheme.DARK);
        return;
      }
    }
    ...
    this._applyCssTheme(cssTheme);
  },
  _applyCssTheme(cssTheme) {
    try {
      ...
      console.error(`_applyCssTheme: "${reason?.message}".`);
    }
  },
